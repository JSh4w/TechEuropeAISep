...
- **Resolved: keep one activity per stage (task 2.6).** The `add-suitability-engine` design promised per-model-call activities via `TemporalAgent`, but in practice stages were always run as cohesive stage activities (`local_sentiment`, `financial_model`, etc.). Running agents from workflow code would complicate stage encapsulation (requiring separate micro-activities for cache and disk I/O) and depends on `TemporalAgent`, which is marked deprecated in pydantic-ai. The earlier design docs and tasks have been corrected to reflect one activity per stage.
