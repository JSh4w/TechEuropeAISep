...
- [x] 2.4 Add the research agent with Gemini and `WebSearch`; verify the stage activity (`local_sentiment`) executes cleanly within Temporal and handles cache hits/misses
...
- [x] 3.4 Add the analyst agent (`evaluate_case` tool, `Recommendation` output); verify the rationale names the chosen duration and quotes only numbers present in the three cases
